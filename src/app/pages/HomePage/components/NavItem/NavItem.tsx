  ...
  return (
    <div className="relative m-2 md:m-4 lg:m-6 self-center rounded overflow-hidden shadow-lg hover:ring-2 ring-light transition-all ease-in-out duration-300">
      <button
        ...
        className={`${styles.card} text-2xl p-2 md:p-4rounded relative`}
        data-testid="NavItem"
      >
        <div className="p-2 md:p-4 lg:px-6 lg:py-4">
          <FontAwesomeIcon icon={["fas", props.item.icon as IconName]} />
          {props.item.name ? <span className="px-4"> {props.item.name} </span> : <></>}
        </div>
      </button>
      {props.editMode ? (
        <>
          <div className="inline absolute top-0 right-0 z-10 cursor-pointer">
            <FontAwesomeIcon
              ...
              className="text-base mr-2 hover:text-lg transition-all ease-in-out duration-200"
              icon={["fas", "pen-to-square"]}
            />
          </div>
          <div className="inline absolute top-0 right-8  z-10 cursor-pointer">
            <FontAwesomeIcon
              className="text-base ml-2 hover:text-lg transition-all ease-in-out duration-200"
              icon={["fas", "minus"]}
            />
          </div>
        </>
